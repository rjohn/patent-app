/**
 * Shared utilities for fetching patent full text (abstract + claims)
 * from USPTO ODP CLM XML and EPO OPS.
 * Used by both the portfolio claims endpoint and watchlist refresh.
 */

const ODP_BASE = 'https://api.uspto.gov/api/v1/patent/applications'

export function odpHeaders(): Record<string, string> {
 ...
 */
export async function fetchClaimsFromOdp(appNumber: string): Promise<{
  ...
  try {
    ...
    // Find the most recent CLM document with XML mime type
    const clmDoc = docBag
    ...
    console.error('[patent-fulltext] ODP CLM fetch error:', e)
    return null
  }
}
...
export async function getEpoToken(): Promise<string | null> {
...
export async function fetchAbstractFromEpo(patentNumber: string): Promise<string | null> {
